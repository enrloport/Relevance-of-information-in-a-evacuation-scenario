...
to setup
  ...
  ask patches with [(abs pxcor = x-dimension and abs pycor <= y-dimension) or (abs pycor = y-dimension and abs pxcor <= x-dimension)]
  [ set pcolor white ]

  ;; set other obstacles
  ask patches with [pycor = y-dimension - 2 and abs pxcor <= 0 ]
  [ set pcolor white ]

  ;; create exits
  ask patches with [(pycor = y-dimension and abs pxcor = x-dimension - doors-position - 1 ) or (pycor = y-dimension and pxcor = x-dimension - doors-position - 2 )]
  [ set pcolor green ]
  ...
  [ ;find a position in the room
    let x1 random x-dimension
    let x2 random-float 1
    if x2 < 0.5 [set x1 -1 * x1]
    let y1 random y-dimension
    let y2 random-float 1
    if y2 < 0.5 [set y1 -1 * y1]
    ;; initial position not over an obstacle
    ifelse any? patches with [pcolor = white and pxcor = x1 and pycor = y1] [set x1 y1 - 1] [setxy x1 y1]

    ifelse random-float 1 < 0.5 [ set in-panic? false ][ set in-panic? true ]

    ;; give the people an initial nudge towards the goal
...
to go
  ...
  ;; run the social forces model on in-panic people
  ;; calculate the forces first...
  ask people with [ in-panic? = true ]
  [
  ...
  ;; then move the people and have them grow impatient if need be
  ask people with [ in-panic? = true ]
  [
    move-turtle
    if get-impatient?
  ...
  ;; control the arrival of new in-panic people
  if any? people with [in-panic? = false] [ wait-around ]
...
;; control when newly in-panic people arrive. follow an exponential distribution of inter-arrival times
to wait-around
  ask one-of people with [in-panic? = false][
    if random-float 1 > 0.25 [ set in-panic? true ]
  ]
end
...
to calc-territorial-forces
  ...
  ask other people with [distance myself > 0]
  [
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "ifelse evacuated > 0\n[plot evacuated * 25 / ticks ]\n[plot 0]"
