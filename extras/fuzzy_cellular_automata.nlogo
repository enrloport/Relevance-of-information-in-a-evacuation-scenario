...
residents-own [dangerous dangerous-location? dis]
attackers-own [dangerous]
fires-own [size dangerous]
patches-own [danger]
...
to setup
  ...
  create-residents number-of-residents [
    setxy (random 100) - 50 (random 100) - 50
    set dangerous false
    set color blue
    set shape "circle"
  ]
  create-attackers number-of-attackers [
    setxy (random 100) - 50 (random 100) - 50
    set color black
    set dangerous true
    rt random 360
  ]
  ask one-of attackers [
    setxy attackers-x-position attackers-y-position
  ]
  create-fires 1 [
    setxy fire-x-position fire-y-position
    set color orange
    set size 1
    set dangerous true
  ]
...
to go
  ...
  ask attackers [
    move-attacker
    ;move-turtle
    if [pcolor] of patch-here = orange [move-to min-one-of patches [distance myself]]
  ]

  ask fires [
    ifelse size < fire-max-expansion and ticks mod (101 - fire-propagation) = 0 [
      set size size + 1
    ][
      set size fire-max-expansion
    ]
    let aux size
  ]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;,,
  ask residents [
    ;let dis distance (min-one-of turtles with [dangerous] [distance myself])
...
;    create-fuzzy-sets
    ask attackers [
      ask myself [
        set dis distance myself
        compute-suitability dis danger-level
        ifelse degree-of-consistency-R2 > degree-of-consistency-R1 [
          set color yellow
          let actual-danger [danger] of patch-here
          if any? neighbors with [not any? turtles-here and danger > actual-danger][
            move-to max-one-of neighbors with [not any? turtles-here and danger > actual-danger] [danger]
          ]
          ;uphill danger
...
to update-world
  let dis1 distance (min-one-of attackers [distance myself])
  let nearest-fire  (min-one-of fires [distance myself])
  let dis2 distance nearest-fire

  set pcolor black
  ifelse dis2 < [size] of nearest-fire [
    set danger dis1 * 0.5
    set pcolor orange
  ][
    set danger dis1
...
to move-attacker
  if any? residents in-radius 5 [
    ask one-of residents in-radius 5 [die]
    set deads deads + 1
  ]

  let destiny one-of patches
  if any? residents [
    let target min-one-of residents [distance myself]
    if any? neighbors with [not any? turtles-here and pcolor != orange][
      set destiny min-one-of neighbors with [not any? turtles-here and pcolor != orange] [distance target]
      face destiny
      move-to destiny
    ]
  ]
...
to move-turtle
  ...
  if not any? neighbors in-cone 1 180 [ rt 180 ]

  ifelse any? neighbors with [pcolor != orange and not any? turtles-here ] in-cone 1 270[
    set destiny one-of neighbors  with [not any? turtles-here  and pcolor != orange ] in-cone 1 270
    ...
    if [pcolor] of patch-here = orange [
      ...
      move-to min-one-of patches with [pcolor != orange and not any? turtles-here] [distance myself]
    ]
  ]
end
...
;  set low-risk        fuzzy:piecewise-linear-set [[0 1] [200 0]]
;  set high-risk       fuzzy:piecewise-linear-set [[0 0] [200 1]]

  set low-risk        fuzzy:gaussian-set [0   20 [0 100]]
  set high-risk       fuzzy:gaussian-set [100 10 [0 100]]

  set close-to-me     fuzzy:gaussian-set [0   15 [0 100]]
  set far-from-me     fuzzy:gaussian-set [100 15 [0 100]]
...
to compute-suitability [#dist #risk-level]
  ...
  set reshaped-consequent-R1 (runresult (word "fuzzy:" "truncate" " not-in-danger degree-of-consistency-R1"))

  ;; Rule 2: ... THEN Suitability is Low.
  set reshaped-consequent-R2 (runresult (word "fuzzy:" "truncate" " in-danger degree-of-consistency-R2"))
