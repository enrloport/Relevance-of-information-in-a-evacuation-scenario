...
to go
  ...
  ask fires [
    if size < fire-max-expansion and ticks mod (101 - fire-propagation) = 0 [
      set size size + 1
    ]
    let aux size
  ]
  ...
  ask residents [
    ...
    ifelse dis < 15 or distance nearest-fire <= [size] of nearest-fire [
      uphill danger
    ][
      move-turtle
    ]
...
to update-world
  let dis distance (min-one-of attackers [distance myself])
  let nearest-fire  (min-one-of fires [distance myself])
...
to move-attacker
  ...
  if any? residents [
    ...
    set destiny min-one-of neighbors with [not any? turtles-here ] [distance target]
    face destiny
    move-to destiny
  ]
end
...
to move-turtle
  ...
  if any? neighbors  with [not any? turtles-here ] in-cone 1 270 [

    set destiny one-of neighbors  with [not any? turtles-here ] in-cone 1 270
