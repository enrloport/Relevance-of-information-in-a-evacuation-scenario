...
to peaceful-desire
  ifelse app-info? and app and app-trigger and route = [] [  ; The app is giving information to the agent.
    ...
    if not in-secure-room? and percived-risk > 0 [
      ...
      ifelse empty? route [
        let loc-aux location
        ifelse any? violents with[ member? location ([reacheables] of loc-aux) ] and ([hidden-places - hidden-people] of location)  > 0  [
          set state "hidden"
        ][
          ifelse any? violents with [loc-aux = location] and ([residents] of location) > ( 11 * count (violents with [loc-aux = location]) )
          [set state "fighting"]
...
to search-intuitive-node
  ...
  ;; TO DO: priorizar el salir de la linea de visión del enemigo
...
to update-world
  ...
  ask violents [
    if detected > 0 [
      ...
      ask nodes with [id - floor id < 0.099][set nearest-danger (distance myself)]
      ask location [
        set habitable 0
  ...
  if any? nodes with [habitable < 1][
    set not-recomended-nodes (remove-duplicates ( reduce sentence ([ [id] of reacheables] of (nodes with [habitable = 0]) ) ) )
  ]
end
